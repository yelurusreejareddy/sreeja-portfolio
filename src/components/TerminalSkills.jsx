...
function TerminalGroup({ name, skills, delay }) {
  ...
  return (
    <div className="font-mono text-sm">
      <p className="text-[#d3915f] mb-2 min-h-5">
        {text.slice(0, shown)}
        {!done && <span className="inline-block w-2 h-4 bg-white/60 animate-pulse align-middle ml-0.5" />}
      </p>
          ...
          <div className="p-6 sm:p-8 space-y-5">
            ...
            {inView &&
              GROUPS.map(([name, skills], i) => (
                <TerminalGroup key={name} name={name} skills={skills} delay={i * 280} />
              ))}
          </div>
        </motion.div>
      </div>
